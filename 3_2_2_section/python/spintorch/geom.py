...
class WaveGeometry(nn.Module):
    def __init__(self, dim: tuple, d: tuple, B0: float, Ms: float, damping_with: int):
        super().__init__()
        ...
        self.register_buffer("damping_with", tensor(damping_with))
...
class WaveGeometryMs(WaveGeometry):
    def __init__(self, dim: tuple, d: tuple, Ms: tuple, B0: float, damping_with: int):

        super().__init__(dim, d, B0, Ms, damping_with)

        self.beta = 1-(self.Ms[2]/self.Ms[1])    ## 1 - Ms_min/Ms_max
        x = torch.linspace(-10,10,2000)
        sigm_Ms = (1/(1+torch.exp(-x))*self.beta+(1-self.beta))*Ms[1]
        min_idx = torch.argmin(abs(sigm_Ms-Ms[0]))
        rho0 = x[min_idx]

        self.rho = nn.Parameter(ones(400,500)*rho0) ##
        self.register_buffer("Msat", zeros(dim))
        self.register_buffer("Msat0", ones(dim)*self.Ms[0]) ##
        self.register_buffer("B0", tensor(B0))
        self.register_buffer("B", zeros((3,)+dim))
        self.B[2,] = self.B0
        
    def forward(self):
        self.rhoPad = torch.ones_like(self.Msat0)*self.Msat0 ##
        self.rhoPad[60:460,120:620] = self.Ms[1]*(torch.sigmoid(self.rho)*self.beta+(1-self.beta)) ##
        self.Msat = self.rhoPad
        return self.Msat
