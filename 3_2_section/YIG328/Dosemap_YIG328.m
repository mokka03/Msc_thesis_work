...
YIG331.d = 99e-9;  % film thickness
YIG331.theta = 0;  % H0 angle, 0 if oop
YIG331.Bext = 214.3e-3;  % applied field in T

%% Measured data

dose_levels = (0:15)*1e12;
dose_levels = dose_levels/1e12;
load('data/wavelength_2050MHz.mat')
YIG331.lambda = wavelength;
YIG331.f0 = ones(size(YIG331.lambda))*2.05e9;
...
% SaveFig('figure/','Wavelength_2050MHz',gcf);
...
% SaveFig('figure/FIBregions_2050MHz_m5dbm_Hpos95p7/','dose_Ms',gcf);

%%
min(min(Ms_levels))
max(max(Ms_levels))
dose_Ms_data = [dose_levels*1e12; Ms_levels];
% save('data/dose_Ms_data_2050MHz.mat','dose_Ms_data');